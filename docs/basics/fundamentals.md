...
| 項目                 | var                                | let                                              |
| -------------------- | ---------------------------------- | ------------------------------------------------ |
| スコープ             | 関数内であればどこでもアクセス可能 | その変数が宣言されたブロック内でのみアクセス可能 |
| ホイスティング(参照) | 変数が宣言される前に参照できる     | 変数が宣言される前に参照できない                 |
| 再宣言               | 同じ名前の変数を再宣言できる       | 同じ名前の変数を再宣言できない                   |

let の方がスコープが明確な上により厳密なエラーチェックを行うことができるのでより安全で予測可能なコードを書くことができる
